...
def user_ExtForces(PxF, RxF, VxF, OMxF, AxF, OMPxF, mbs_data, tsim, ixF):
    ...
    wind_sensors = [
        mbs_data.extforce_id['wind_nacelle1'],
        mbs_data.extforce_id['wind_nacelle2'],
        mbs_data.extforce_id['wind_nacelle3'],
        mbs_data.extforce_id['wind_nacelle4'],
    ]

    if ixF in wind_sensors:
        A = 300.0       
        omega = 6 * np.pi 

        F_wind = A * (1 - np.cos(omega * tsim))
